...
// Skip entire file if no database connection (avoids import error)
if (!process.env.POSTGRES_URL) {
  describe.skip('POST /api/upload integration (no POSTGRES_URL)', () => {
    it.skip('skipped - no database connection', () => {});
  });
} else {
  // Dynamic imports to avoid loading database without connection string
  const { POST } = await import('./route');
  const { NextRequest } = await import('next/server');
  const { db, images } = await import('@lens-llama/database');
